...
async function TiktokData(url) {
    ...
    return ({
		...
		código: 200,
		criador: "@Ꮥꪖꪗꪮ </>",
		título: videoMeta.collector[0].text,
		vídeo_URL: {
...
const Tiktok = (url) => new Promise((resolve, reject) => {
    if (url === 'undefined') { reject('digite o texto mano.') }
    try {
...
const FB = (url) => new Promise((resolve, reject) => {
	if(!url || url == undefined) return reject("Coloque o link");
	fbdl.getInfo(url)
	.then(res => {
		resolve({
			title: res.title,
